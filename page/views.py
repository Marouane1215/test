...
def sist_gantt(request):
    ...
    for j in range(num_jobs):
        st = setup_times[0] if j > 0 else 0
        p = int(jobs_details[j]["machine_times"][0]["preparation_time"])
        s[0][j] = max(int(jobs_details[j]["arrival_date"]), c[0][j - 1] if j > 0 else 0) + st
        c[0][j] = s[0][j] + p
    ...
    for i in range(1, num_machines):
        for j in range(num_jobs):
            st = setup_times[i]
            p = int(jobs_details[j]["machine_times"][i]["preparation_time"])
            s[i][j] = max(c[i - 1][j], c[i][j - 1] + st if j > 0 else 0)
            c[i][j] = s[i][j] + p
    ...
    for i in range(num_machines):
        for j in range(num_jobs):
            ...
            preparation_end = s[i][j] + setup_times[i]

            job_id = jobs_details[j]["job_id"]

            # Période de préparation (hachures)
            ax.barh(f"Machine {i + 1}", preparation_end - start, left=start,
                    color="none", edgecolor="black", hatch='//', label="Préparation" if i == 0 and j == 0 else "")

            # Période de traitement
            ax.barh(f"Machine {i + 1}", end - preparation_end, left=preparation_end,
                    color=job_colors[job_id], edgecolor="black", label=f"Job {job_id}" if i == 0 else "")

    ax.set_xlabel("Temps")
    ax.set_ylabel("Machines")
    ax.set_title("Diagramme de Gantt - SDST (Préparation colorée)")
    ax.grid(axis="x", linestyle="--", alpha=0.7)
